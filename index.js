        ...
        <div onClick={e=>e.target===e.currentTarget&&setModalOpen(false)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:1000,display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
          <div style={{background:'#fff',borderRadius:'20px 20px 0 0',padding:'20px',width:'100%',maxWidth:'480px',maxHeight:'85vh',overflowY:'auto'}}>
            ...
            <div style={{display:'flex',gap:4,marginBottom:14}}>
              <button onClick={()=>setAddTab('url')} style={{flex:1,padding:'8px',borderRadius:10,fontSize:13,fontWeight:500,border:'1px solid #E8E6E0',background:addTab==='url'?'#2C2C2A':'#F7F6F2',color:addTab==='url'?'#fff':'#888780',cursor:'pointer',fontFamily:'inherit'}}>🔗 쇼핑몰 URL</button>
              <button onClick={()=>setAddTab('photo')} style={{flex:1,padding:'8px',borderRadius:10,fontSize:13,fontWeight:500,border:'1px solid #E8E6E0',background:addTab==='photo'?'#2C2C2A':'#F7F6F2',color:addTab==='photo'?'#fff':'#888780',cursor:'pointer',fontFamily:'inherit'}}>📷 사진 업로드</button>
            </div>

            {addTab === 'url' && (
              <div>
                <div style={{display:'flex',gap:8,marginBottom:10}}>
                  <input value={shopUrl} onChange={e=>setShopUrl(e.target.value)} placeholder="무신사, 29CM 등 상품 URL" style={{flex:1,border:'1px solid #E8E6E0',borderRadius:10,padding:'9px 12px',fontSize:13,fontFamily:'inherit',outline:'none'}}/>
                  <button onClick={fetchFromUrl} disabled={urlLoading} style={{padding:'9px 14px',borderRadius:10,fontSize:13,fontWeight:500,border:'none',background:'#2C2C2A',color:'#fff',cursor:'pointer',fontFamily:'inherit',flexShrink:0}}>
                    {urlLoading ? '...' : '가져오기'}
                  </button>
                </div>
            ...
            <div style={{display:'flex',gap:8,marginTop:16}}>
              <button onClick={()=>setModalOpen(false)} style={{flex:1,padding:'11px',borderRadius:10,fontSize:13,fontWeight:500,border:'1px solid #E8E6E0',background:'#fff',cursor:'pointer',fontFamily:'inherit'}}>취소</button>
              <button onClick={saveCloth} style={{flex:1,padding:'11px',borderRadius:10,fontSize:13,fontWeight:500,border:'none',background:'#2C2C2A',color:'#fff',cursor:'pointer',fontFamily:'inherit'}}>저장</button>
            </div>
          </div>
        </div>
      )}
